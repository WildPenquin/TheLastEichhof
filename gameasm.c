...
void defshot (int shot, int x, int y, struct anishot *addr) {
  ...
  if (ptr - (struct anishot *) &_shot == MAXSHOTS)
    return;
...
void deffoe (short foe, short x, short y, short x0, short y0) {
  ...
  ptr->object = defobject (intindex[lsprofs + fs->sprite], x, y, OBJ_LOW);
}
...
void a_shot (void) {
  ...
  nextcoord:
	...
	if (subtick != 0)
	  continue;
	defshot (ptr->data[1], _obj[ptr->object].xa, _obj[ptr->object].ya,
		 ptr);
	ptr->data += 2;
	goto nextcoord;
      case SHOTHOMING:
	{
	  ...
	  do {
	    if (f->object == -1)
	      continue;
	    if (f->flags & (FOE_INVINCIBLE | FOE_TRANSPARENT))
	      continue;
	    break;
	  } while (f++, --fctr);
...
void a_foe (void) {
  ...
  do {
	...
	switch ((unsigned short) *ptr->cpath) {
	case FOEENDPATH:
	...
	case FOERELEASEFOE:
	  ...
	  deffoe (ptr->cpath[1],
		  ...
		  _obj[ptr->object].ya + ptr->cpath[3], 0, 0);
	  ptr->cpath += 4;
	  goto nextcoord;
	case FOECYCLEPATH:
	  ptr->cpath = ptr->path;
...
void a_expl (void) {
  ...
  nextcommand:
    ...
    case EXPLREMOVEOBJ:
      ...
      goto nextcommand;
    case EXPLRELEASEFOE:
      deffoe (ptr->data[1], ptr->x + (signed short) ptr->data[2], ptr->y + (signed short) ptr->data[3], 0, 0);	/* No arguments here in original */
      ptr->data += 4;
      goto nextcommand;
    case EXPLNEWPATH:
      deffoe (ptr->data[1],
...
fire_label:
  do {
	...
	defshot (ptr->shot,
		 _obj[ptr->object].xa,
		 _obj[ptr->object].ya, (struct anishot *) -1);
      }
    }
