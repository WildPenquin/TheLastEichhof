...
// Enter name
  ...
  if (inlist) {
    ...
    //setvect(0x09, int09);
    do {
      cursorblink (x, YOFFS + currentpl * 14, hstxt.player[currentpl].name[i],
		   font);
      /* Read ASCII value */
      keys = readkey () & 0xFF;
      if (keys <= 122 && keys >= 97)
    ...
    if (currentpl == 0) {
      writetext (64, YOFFS + (MAXENTRIES + 1) * 14 + 8,
		 "PLEASE ENTER WINNER TEXT", font);
      i = 0;
      xs = font2->xs;
      do {
	if (i == NOCW) {
	  x = (320 + (NOCW - 3) * xs) / 2;
	  cursorblink (x, YOFFS + (MAXENTRIES + 3) * 14 - 6,
		       hstxt.winnertext[i - 1], font2);
	} else {
	  x = (320 + (i - 1) * xs) / 2;
	  cursorblink (x, YOFFS + (MAXENTRIES + 3) * 14 - 6,
		       hstxt.winnertext[i], font2);
	}
	keys = readkey () & 0xFF;
	if (keys <= 122 && keys >= 97)
	  keys -= 32;
	if ((keys <= 93 && keys >= 65) || (keys <= 59 && keys >= 39) ||
	  ...
	  writetext ((320 - (i + 1) * xs) / 2,
		     YOFFS + (MAXENTRIES + 3) * 14 - 6, hstxt.winnertext,
		     font2);
	} else if (keys == 0x08 && i > 0) {
	  i--;
	...
	writetext (x, YOFFS + (MAXENTRIES + 3) * 14 - 6, hstxt.winnertext,
		   font2);
      }
...
// Not in highscore
  ...
  unloadfile (font2_file);
  free (font2);
  unloadfile (font_file);
  free (font);
}
