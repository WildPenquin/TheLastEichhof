...
void displifes (void) {
  int i;
  for (i = 1; i <= 6; i++)
    putspritedirect (barfont, BARLIFEX - i * 4, BARSCOREY, BARCLRDIGIT);
  ...
  for (; i >= 0; i--)
    putspritedirect (barfont, BARLIFEX - i * 4, BARSCOREY, BARLIFEDIGIT);
}
...
// Check for wrong level.
  if (level.descript->level != stage)
    error ("initlevel", EINVAL);
