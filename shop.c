...
// Display screen and begin placing
  ...
  do {
    ...
    if (key[key_up]) {
      y -= 4;
      if (y < ((WEAPONY0 + 4) & 0xfffc))
	y += 4;
      writetext (WEAPONXTEXT, WEAPONYTEXT, "       ", font);
    }
    if (key[key_down]) {
      y += 4;
      if (y > ((WEAPONY1 - obj_ys - 4) & 0xfffc))
	y -= 4;
      writetext (WEAPONXTEXT, WEAPONYTEXT, "       ", font);
    }
    if (key[key_left]) {
      x -= 4;
      if (x < ((WEAPONX0 + 4) & 0xfffc))
	x += 4;
      writetext (WEAPONXTEXT, WEAPONYTEXT, "       ", font);
    }
    if (key[key_right]) {
      x += 4;
      if (x > ((WEAPONX1 - obj_xs - 4) & 0xfffc))
	x -= 4;
      writetext (WEAPONXTEXT, WEAPONYTEXT, "       ", font);
    }
    if (key[key_fire]) {
...
// Turn Exitsign out
  if ((*exit) < -2) {
    ...
    removesprite (exitspr, SELLXEXIT, SELLYEXIT);
    if ((*exit) < -3)
      putsprite (exitspr, SELLXEXIT, SELLYEXIT, -(*exit) - 4);
  }
}
...
// Write Money the Player has and title
  ...
  writetext (160 - (15 * font->xs), 0, "CHOOSE WEAPON YOU WANT TO SELL",
	     font);
...
// Flash Weapon
      if (selected != 0) {
	objflashcolor += dir;
	if ((objflashcolor < WEAPONFLASH)
	    || (objflashcolor > WEAPONFLASH + 15)) {
	  dir = -dir;
	  objflashcolor += dir;
	}
...
static int checkforexit (int selected, struct sprstrc *font) {
  ...
  if (!(weapon.arm[selected].flags & W_ISWEAPON))
    return TRUE;
  if (nweapons >= MAXARMS) {
	...
	if (selected < weapon.narms - min (weapon.narms, SHOPMAXARMS) + 1) {
	  ...
	  if (selected == (narms + 1))
	    delta = ye - yw - ((narms - 1) * (font1->ys + 3));
		   ...
		   weapon.narms - min (weapon.narms, SHOPMAXARMS) + 1)
...
// Making Menucard
	...
	for (i = min (weapon.narms, SHOPMAXARMS); i < weapon.narms; i++) {
	  if (weapon.arm[i].flags & W_ISWEAPON) {
	    memset (text, '.', 28);
	    text[28] = '\0';
	    memcpy (text, weapon.arm[i].armname,
		    strlen (weapon.arm[i].armname));
	    writetext (MENUX, y, text, font1);
	    writenumber (MENUXPRICE, y, weapon.arm[i].cost, font1);
	    y += font1->ys + 3;
	  }
...
static void placeprice (long price, struct sprstrc *font) {
  ...
  for (; j < 5; j++) {
    ...
    if (zahl != 0 || j == 2) {
      ...
      if (i != 0)
	for (; i > 0; i--)
	  putspritedirect (font, x + i * xs, SHOPYPRICE, 0);
      i = 0;
    } else {
...
static void closeshop (struct sprstrc *pf, struct sprstrc *f, int obj,
		       int espr, int pspr) {
  glowout ();
...
// Move pointer according to the pressed keys.
  ...
  do {
	...
	j = _sprite[intindex[weapon.arm[selected - 1].sprite]].xs / 2
	  + _sprite[intindex[weapon.arm[selected].sprite]].xs / 2 + SHOPSPC;
	for (i = 0; i < j; i += 2) {
	  x += 2;
	  moveobjectdelta (obj, 2, 0);
	  waitfortick ();
	  updatescreen ();
	}
	writetext (0, SHOPTEXT, weapon.arm[selected].armname, font);
...
// Player has pressed fire.
  ...
  case 0:
    ...
    return FALSE;
  default:
    payforweapon (selected, font);	// Let Player pay for new weapon
    closeshop (pfont, font, obj, exitspr, pointspr);
